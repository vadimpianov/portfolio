...
void main() {
  ...
  // Сдвиг по кольцу (поперёк лент) — только вперёд и неторопливо, чтобы узор не
  // «выдавливало» вверх; скорость плавает, но всегда > 0
  // (производная фазы: 0.006 − 0.0033 − 0.00065 > 0).
  float phase = th * 0.006 + 0.03 * sin(th * 0.11) + 0.015 * sin(th * 0.043 + 1.3);
  ...
  for (int i = 0; i < 4; i++) {
    float fi = float(i);
    // Течение вдоль лент (как жидкость по руслу): изгибы бегут по касательной к лентам,
    // слои с разной скоростью скользят друг по другу — меандры и завихрения.
    vec2 tangent = vec2(-dir.y, dir.x);
    vec2 drift = tangent * th * (0.035 + 0.02 * fi);
    drift += 0.35 * vec2(snoise(vec3(th * 0.03, fi * 7.1, 0.0)), snoise(vec3(th * 0.03, fi * 7.1, 9.0)));
    vec2 q = (w - drift) * 0.6 + fi * 3.7;
    w += 0.22 * vec2(snoise(vec3(q, t * 0.08 + fi)), snoise(vec3(q + 5.2, t * 0.08 + fi)));
  }
