...
void main() {
  ...
  // «Густой поток»: цветные ленты текут в одну сторону (dir) и плавно изгибаются.
  ...
  for (int i = 0; i < 4; i++) {
    float fi = float(i);
    // Изгибы плывут вместе с потоком и медленно меняются.
    vec2 q = (w - dir * th * 0.02) * 0.9 + fi * 3.7;
    w += 0.25 * vec2(snoise(vec3(q, t * 0.04 + fi)), snoise(vec3(q + 5.2, t * 0.04 + fi)));
  }

  // ~1.2 круга палитры по диагонали экрана: в каждом кадре все цвета в нужной
  // пропорции; сдвиг по кольцу — только вперёд, с постоянной скоростью.
  float v = dot(w, dir) * 0.6 - th * 0.012;
  // Положение курсора по горизонтали слегка сдвигает всю палитру.
  v += (uMouse.x - 0.5) * 0.15 * uHover;

  // Растворение: усредняем соседние оттенки палитры — цвета мягко перетекают.
  vec3 color = vec3(0.0);
  for (int k = -3; k <= 3; k++) {
    color += palette(v + float(k) * 0.022);
  }
  color /= 7.0;
