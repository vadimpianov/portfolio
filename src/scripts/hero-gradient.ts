...
void main() {
  ...
  float dirAngle = 1.22 + 0.35 * snoise(vec3(th * 0.012, 3.3, 1.1));
  ...
  // Плотность лент подстраивается под направление: поперёк экрана ~0.88 круга (меньше одного) — каждый цвет
  // виден одним куском, без повторов.
  float extent = aspect * abs(dir.x) + abs(dir.y);
  float density = 0.88 / extent;

  // Изгибы: 4 последовательные слабые деформации. Каждая сама по себе обратима
  // (A·F·max|∇шума|·√2 ≈ 0.5 — с запасом вдвое: при ≈0.9 поток почти «складывался»
  // и граница ленты заострялась в пики), значит и цепочка обратима — поле без вершин
  // и впадин. Изгибы крупные и плавные (низкая частота), как разводы густой жидкости.
  // Бегущие волны: ленты колышутся синусоидой, волна бежит вдоль них. Сдвиг идёт
  // поперёк лент и зависит только от координаты вдоль них — это сдвиг-«ножницы»
  // (определитель = 1), поток не складывается: ни дырок, ни острых пиков.
  vec2 tangent = vec2(-dir.y, dir.x);
  float along = dot(p, tangent);
  float wave = 0.06 * sin(along * 5.0 - th * 0.35) + 0.03 * sin(along * 8.5 + th * 0.22 + 1.7);
  vec2 w = p + dir * wave;
  for (int i = 0; i < 4; i++) {
    float fi = float(i);
    // Одно общее течение: изгибы переносятся вместе с цветом (как вещество в потоке),
    // слои лишь чуть отличаются по скорости — это даёт объём, а не «синтетику».
    vec2 drift = dir * (phase / density) * (0.85 + 0.1 * fi);
    drift += 0.25 * vec2(snoise(vec3(th * 0.02, fi * 7.1, 0.0)), snoise(vec3(th * 0.02, fi * 7.1, 9.0)));
    vec2 q = (w - drift) * 0.6 + fi * 3.7;
    w += 0.22 * vec2(snoise(vec3(q, t * 0.05 + fi)), snoise(vec3(q + 5.2, t * 0.05 + fi)));
  }

  float v = dot(w, dir) * density - phase;
