...
void main() {
  ...
  // Изгибы плывут вместе с потоком и медленно меняются — как густая жидкость.
  vec2 flowP = (p - dir * th * 0.02) * 0.6;
  vec2 w = p + 0.3 * vec2(snoise(vec3(flowP, t * 0.04)), snoise(vec3(flowP + 5.2, t * 0.04)));
  // Курсор мягко раздвигает поток вокруг себя (без собственного цвета).
  w += (p - m) * influence * 0.2;
